...
// Marker with hover-to-open popup
...
export default function App() {
  ...
  const handleMapPick = (latlng) => {
    ...
    if (!isWithinCampus(latlng.lat, latlng.lng)) {
      setBoundaryError("📍 That location is outside UofT St. George campus. Please pick a spot within campus.");
      setShowListingModal(true);
      pickingRef.current = false;
      setPickingLocation(false);
      return;
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(false);
    if (error) {
      setFormMsg("Error saving listing. Please try again.");
    } else {
      setFormMsg(editingId ? "✓ Listing updated!" : "✓ Listing added to the map!");
      loadListings();
      setTimeout(() => { setShowListingModal(false); setFormMsg(""); setEditingId(null); }, 1500);
    }
  };
  ...
  return (
    <div className="app">
      <header className="header">
        ...
        <input className="search" type="text" placeholder="Search groups, food, buildings..."
          value={search} onChange={(e) => setSearch(e.target.value)} />
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {user ? (
            <>
              <button className="auth-btn apply" onClick={openNewListing}>+ Add Listing</button>
              <span style={{ fontSize: 11, color: "#666", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.email}</span>
              <button className="auth-btn" onClick={handleSignOut}>Sign out</button>
            </>
          ) : (
            <button className="auth-btn" onClick={() => setShowLogin(true)}>Sign in to sell</button>
          )}
        </div>
      </header>
      ...
      <div className="main">
        <aside className="sidebar">
          ...
          <div className="place-list">
            {loading ? (
              <div className="loading-msg">Loading listings...</div>
            ) : visible.length === 0 ? (
              <div className="loading-msg">No listings yet. Be the first to add one!</div>
            ) : (
              visible.map((p) => {
                  ...
                  <div key={p.id} className={`place-item ${selected?.id === p.id ? "active" : ""}`}
                    ...
                    <div className="place-info">
                      <div className="place-name">
                        {p.group_name}
                        {isOwner && (
                          <button className="edit-btn" onClick={(e) => { e.stopPropagation(); openEditListing(p); }} title="Edit listing">✏️</button>
                        )}
                      </div>
        ...
        <div className="map-wrap">
          {pickingLocation && (
            <div className="pick-banner">
              📍 Click anywhere on campus to place your listing pin
              <button onClick={cancelPicking}>Cancel</button>
            </div>
          )}
          <MapContainer center={[43.6629, -79.3957]} zoom={15} style={{ width: "100%", height: "100%" }}>
        ...
        <div className="modal-overlay" onClick={() => setShowListingModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-row">
              ...
              <input type="text" placeholder="e.g. UofT Engineering Society"
                value={form.group_name} onChange={e => setForm(f => ({ ...f, group_name: e.target.value }))} />
            </div>
            <div className="form-row">
              <label>What are you offering? *</label>
              <input type="text" placeholder="e.g. Jerk chicken plates, bake sale, free pizza"
                value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
            </div>
            <div className="form-row">
              <label>Price (leave blank for Free)</label>
              <input type="text" placeholder="e.g. $12, $3-$8, Pay what you can"
            ...
            <div className="form-row">
              ...
              <input type="text" placeholder="e.g. Bahen Centre, front entrance"
                value={form.building} onChange={e => setForm(f => ({ ...f, building: e.target.value }))} />
            </div>
            <div className="form-row">
              <label>Available Time *</label>
              <input type="text" placeholder="e.g. Today 11am - 2pm, Friday 12pm - 4pm"
                value={form.available_time} onChange={e => setForm(f => ({ ...f, available_time: e.target.value }))} />
            </div>
            <div className="form-row">
              ...
              {form.lat && form.lng ? (
                <div style={{ fontSize: 12, color: "#1D9E75", marginBottom: 6 }}>
                  ...
                  <button onClick={() => setForm(f => ({ ...f, lat: null, lng: null }))}
                    style={{ marginLeft: 8, fontSize: 11, color: "#aaa", background: "none", border: "none", cursor: "pointer" }}>
                    Change
                  </button>
                </div>
              ) : (
            ...
            <button className="submit-btn" onClick={handleSubmit} disabled={submitting}>
              {submitting ? "Saving..." : editingId ? "Save Changes" : "Add to Map"}
            </button>
            {editingId && (
              <button className="delete-btn" onClick={() => handleDelete(editingId)}>🗑 Delete Listing</button>
            )}
